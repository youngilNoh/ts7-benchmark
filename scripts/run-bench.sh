    ...
    for checkers in 1 4 8; do
        ...
        if [ "$fixture" = "real-world" ]; then
            hyperfine \
                ...
                gtime -v npm run --silent ts6 -- --noEmit -p fixtures/real-world/zod.tsconfig.bench.json 2> "results/$fixture-checkers-ts6-mem.txt"
                gtime -v npm run --silent ts7 -- --noEmit -p fixtures/real-world/zod.tsconfig.bench.json --checkers $checkers 2> "results/$fixture-checkers-$checkers-ts7-mem.txt"
        else
            hyperfine \
                --warmup 2 \
                ...
                --export-json "results/$fixture-bench-checkers-$checkers.json"
                gtime -v npm run --silent ts6 -- --noEmit -p fixtures/synthetic/$fixture 2> "results/$fixture-checkers-ts6-mem.txt"
                gtime -v npm run --silent ts7 -- --noEmit -p fixtures/synthetic/$fixture --checkers $checkers 2> "results/$fixture-checkers-$checkers-ts7-mem.txt"
        fi
    done
done
