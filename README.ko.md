...
**네이티브 TypeScript 7 컴파일러와 TypeScript 6의 타입 체크 성능**을 재현 가능한 방식으로 비교하는 벤치마크입니다. 합성(synthetic) fixture(크기를 조절해 생성)와 커밋 해시로 고정된 실제 오픈소스 프로젝트([zod](https://github.com/colinhacks/zod)) 양쪽에 대해 `tsc --noEmit`을 실행해서 실행 시간(wall-clock time)과 최대 메모리 사용량을 기록합니다. TS7은 병렬로 타입 체크를 할 수 있기 때문에, 새로 생긴 `--checkers` 옵션에 따라 결과가 어떻게 달라지는지도 함께 측정합니다. 수치는 GitHub Actions를 통해 매주 자동으로 갱신되고, 인터랙티브 대시보드로 게시됩니다.
...
인터넷엔 "TS7이 10배 빠르다!"는 검증 불가능한 스크린샷이 넘쳐납니다. 이 저장소는 정반대를 지향합니다:

- **재현 가능함** — 누구나 clone해서 명령어 몇 개만 실행하면 비교 가능한 수치를 얻을 수 있습니다. 컴파일러 버전은 고정되어 있고, 실제 프로젝트 fixture도 정확한 커밋에 고정되어 있습니다.
- **정직함** — 실제로 측정한 것(타입 체크만), 어떤 하드웨어에서, 몇 번 실행했는지를 그대로 보고하고, [한계](#한계)도 숨기지 않습니다.
- **규모를 고려함** — 4가지 fixture 크기와 3가지 `--checkers` 설정에 걸쳐 측정해서, 단순한 헤드라인 숫자 하나가 아니라 **속도 향상이 어디서 오는지**를 볼 수 있습니다.

## 결과

이 글을 쓰는 시점 기준, TS7은 TS6보다 대략 **3~4배** 빠르게 타입 체크를 하며, 이 격차는 **코드베이스가 크고 checker가 많을수록 더 벌어지는 경향**이 있습니다. 합성 fixture는 상대적으로 작고 평평한 속도 향상을 보이고, 복잡한 제네릭을 많이 쓰는 실제 프로젝트(zod)는 checker를 늘릴수록 가장 큰 이득을 봅니다.
...
두 컴파일러 모두 `npx tsc`가 아니라 `package.json`의 스크립트를 통해 실행합니다. 이건 의도된 것입니다: `@typescript/typescript6`을 설치하면 진짜 TypeScript 6이 함께 딸려 들어오는데, 그 TS6과 TypeScript 7 **둘 다 `tsc`라는 실행 파일 이름을 원합니다.** 그래서 `npx tsc`는 그 이름 충돌에서 이긴 쪽(즉 TS6)으로 해석되어버려 신뢰할 수 없습니다. `ts6` / `ts7` 스크립트는 각 컴파일러를 경로로 직접 가리키므로, 어느 쪽이 실행되는지 절대 모호하지 않습니다.

### 왜 TS7에는 `--checkers` 옵션이 있고 TS6에는 없는가

TypeScript 7은 **컴파일러를 Go로 완전히 새로 작성한 것**입니다 (예전엔 *typescript-go*라고 불렸던 "네이티브" 컴파일러). 이 재작성 덕분에 병렬 타입 체크가 가능해졌습니다:

- TypeScript 5/6은 TypeScript로 작성되어 **단일 스레드** 자바스크립트 런타임 위에서 돌아갑니다. 타입 체커는 심볼, 타입, 추론 결과처럼 서로 공유되고 계속 바뀌는 거대한 상태 그래프를 만드는데, 자바스크립트의 워커(스레드)들은 **아무것도 공유하지 않는(share-nothing)** 구조라서 이 모든 걸 직렬화해서 복사해야 합니다 — 너무 비싸서, 여러 코어로 나눠 검사하는 건 애초에 실용적이지 않았습니다.
- Go는 저렴한 동시성(고루틴)과 **메모리를 공유하는** 모델을 갖고 있어서, 네이티브 컴파일러는 **같은 프로그램 표현을 여러 타입 체크 워커가 동시에** 들여다볼 수 있습니다.
